    ...
    <div style={{ minHeight: '100vh', background: '#f9fafb', display: 'flex' }}>
      ...
      >
        ...
        <div style={{ padding: '0 8px', marginBottom: '28px' }}>
          <h1 style={{ margin: 0, fontSize: '16px', fontWeight: 800, color: NAVY }}>
            E-KALUSUGAN
          </h1>
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: '4px', flex: 1 }}>
          <NavBtn icon="home"        label="Home"         active={activeTab === 'home'}         onClick={() => setActiveTab('home')}         color={INDIGO} />
          <NavBtn icon="queue"       label="My Queue"     active={activeTab === 'queue'}        onClick={() => setActiveTab('queue')}        color={INDIGO} />
          <NavBtn icon="appointment" label="Appointments" active={activeTab === 'appointments'} onClick={() => setActiveTab('appointments')} color={INDIGO} />
        </nav>
        ...
        <div style={{ borderTop: '1px solid #f3f4f6', paddingTop: '16px' }}>
          <div style={{ padding: '0 8px', marginBottom: '8px' }}>
            ...
            <p style={{ margin: '1px 0 0', fontSize: '11px', color: '#9ca3af' }}>Patient</p>
          </div>
      ...
      <main style={{ flex: 1, padding: '28px 24px', maxWidth: '860px' }}>
        ...
        {activeTab === 'home' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            >
              <p style={{ margin: '0 0 4px', fontSize: '13px', color: '#c7d2fe' }}>Welcome back,</p>
              <h2 style={{ margin: '0 0 6px', fontSize: '24px', fontWeight: 800, color: '#ffffff' }}>
                {user?.first_name ?? 'Patient'} 👋
              </h2>
              <p style={{ margin: 0, fontSize: '13px', color: '#a5b4fc' }}>
                Bago City Health Center — {new Date().toLocaleDateString('en-PH', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
              </p>
            </div>
            ...
            <div>
              ...
              {hasActiveQueue && (
                <p style={{ margin: '8px 0 0', fontSize: '12px', color: '#9ca3af' }}>
                  You already have an active queue number. Cancel it first to get a new one.
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px' }}>
                <StatCard icon="queue"       label="Queue Today"       value={hasActiveQueue ? queue.queue_number : '—'} color={NAVY}   />
                <StatCard icon="appointment" label="Upcoming Visits"   value={appointments.filter(a => a.status === 'confirmed').length} color={ORANGE} />
                <StatCard icon="checkCircle" label="Past Consultations" value="—" color={INDIGO} />
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'appointments' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ margin: 0, fontSize: '18px', fontWeight: 700, color: '#111827' }}>Appointments</h2>
            </div>
            {apptLoading ? (
                  ...
                  >
                    <div>
                      ...
                      <p style={{ margin: '2px 0 0', fontSize: '12px', color: '#6b7280' }}>
                        {new Date(appt.date).toLocaleDateString('en-PH', { weekday: 'short', month: 'short', day: 'numeric' })}
                    ...
                    <span
                      style={{
                        ...
                        color:      appt.status === 'confirmed' ? '#059669' : '#6b7280',
                      }}
                    >
                      {appt.status}
                    </span>
