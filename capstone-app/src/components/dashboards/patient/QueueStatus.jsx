...
export default function QueueStatus({ queue, onCancel }) {
  ...
  const config = STATUS_CONFIG[queue.status] ?? STATUS_CONFIG[QUEUE_STATUS.WAITING]
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: 'center', marginBottom: '20px' }}>
        <p style={{ margin: '0 0 6px', fontSize: '13px', color: '#6b7280' }}>Your Queue Number</p>
        <div
          style={{
        ...
        >
          <span style={{ fontSize: '42px', fontWeight: 800, color: config.color, lineHeight: 1 }}>
            {queue.queue_number}
          </span>
          {isPriority && (
            <span
              style={{
                ...
                background: '#fff7ed', borderRadius: '6px', padding: '2px 8px',
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '16px' }}>
        <div
          style={{
            ...
            background: config.bg, borderRadius: '20px', padding: '6px 14px',
          }}
        >
      ...
      {queue.services?.length > 0 && (
        <div style={{ marginBottom: '16px' }}>
          <p style={{ margin: '0 0 8px', fontSize: '12px', color: '#9ca3af', textAlign: 'center' }}>Services</p>
